...
from similarities.utils.util import dot_score
from similarities.utils.util import cos_sim
from similarities.utils.util import semantic_search
...
class BertSimilarity(SimilarityABC):
    ...
    def __init__(
            ...
            model_name_or_path="/data/zyh/similarities/similarities/text2vec-base-chinese",
            device=None,
    ):
        """
        Initialize the similarity object.
        :param model_name_or_path: Transformer model name or path, like:  中文语义匹配模型
    ...
    def most_similar(self, queries: Union[str, List[str], Dict[str, str]], topn: int = 10,
        ...
        """
        根据语料库查找与查询最相似的前n个文本。
        它可以用于语料库的信息检索/语义搜索，最多可达100万个条目。
        ...
        queries_ids_map = {i: id for i, id in enumerate(list(queries.keys()))} # 关键字
